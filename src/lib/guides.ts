...
export const GUIDES: readonly Guide[] = [
  ...
  */
  ...
  /*
    시기 글. 앞의 열두 편은 전부 "누가·무엇을·어떻게"를 다뤘고, **언제**를
    다루는 글은 하나도 없었다.

    네이버 데이터랩으로 지난 1년 검색량을 재 보고 만들었다(2026-09-06).
    근로장려금은 5월이 7월의 5.5배, 청년 지원금은 5월이 7월의 8배였다.
    사람들은 제도가 궁금해서 찾는 게 아니라 **마감이 닥쳐서** 찾는다.

    이 둘은 다른 글과 달리 **수록 데이터에서 뽑아 쓸 수 없다.** 마감일이
    있는 570건 중 554건이 `9999-12-31`(상시)이라 API에 진짜 날짜가 없다.
    그래서 공식 출처를 하나씩 열어 확인한 것만 적었다(lib/calendar.ts에
    항목마다 출처와 확인일을 남긴다).
  */
  {
    slug: "calendar",
  ...
  },
  {
    ...
    /* 제목에 "지급일"을 세운 이유. 네이버 데이터랩으로 재 보니(2026-09-06)
       **"근로장려금 지급일" 한 낱말이** 청년월세 관련 세 낱말을 합친 것의
       ...
       사이트 전체에 한 번도 없었다 — 답은 본문에 다 적어 놓고, 그걸 찾는
       사람이 쓰는 말만 빠져 있었다. */
    title: "근로장려금 지급일 — 5월과 9월은 다른 문입니다",
    summary:
      "5월 정기신청분은 8월 27일에 지급됐고(기한 9월 말), 9월 반기신청분은 12월 17일 지급 예정입니다. 한 해에 신청 창구가 넷인데 이름이 비슷해 헷갈립니다. 언제 신청하면 언제 얼마가 들어오는지 국세청 안내로 정리했습니다.",
    updated: "2026-09-19",
  },
  /*
    지급일 글. 앞의 열넷은 전부 "언제 신청하나"까지만 말했다.

    2026-09-06에 검색량을 재 보고 빈자리를 알았다 — **"근로장려금 지급일"
    한 낱말이** 청년월세 관련 세 낱말을 합친 것의 3배였는데(8월 기준),
    "지급일"이라는 말이 사이트 전체에 한 번도 없었다.

    이것도 수록 데이터에서 뽑을 수 없다. 복지로 상세에 지급일 칸이 없고,
    조회수 상위 120건 본문에 "매월 N일"이 적힌 것은 2건뿐이다. 그래서
    법령을 한 조씩 찾아 옮겼다(lib/payDates.ts에 조문과 확인일을 남긴다).
  */
  {
    slug: "pay-dates",
    /* 처음엔 "20일과 25일에 들어옵니다"였다. 2026-09-07에 법령을 더 뒤져
       **15일**(보훈급여금·참전명예수당)이 나오면서 제목이 틀린 말이 됐다.
       ...
       늘릴 때마다 이 줄을 같이 본다. */
    title: "지원금은 15일·20일·25일에 들어옵니다",
    summary:
      "보훈급여금은 15일, 생계·주거급여와 장애수당은 20일, 기초연금과 아동수당은 25일입니다. 복지로에도 안 적혀 있는 지급일을 법령에서 조문째 찾아 모았습니다.",
    updated: "2026-09-15",
  },
  /*
    실업급여 글. **수록 데이터에 아예 없는 제도**를 다루는 첫 글이다.

    왜 썼나. 네이버 데이터랩으로 재 보니(2026-09-07) 실업급여 검색량이
    문화누리카드급(월평균 35 대 45)인데, 900건을 훑어 제목에 "실업"이 들어간
    사업이 **0건**이었다. 고용보험 소관이라 복지로 API에 애초에 안 들어온다.

    그래서 이 글에는 우리 집계 숫자가 하나도 없다. 「고용보험법」과
    시행령 조문, 고용노동부·고용24 안내만으로 썼다. 자격이 되는지는
    말하지 않는다 — 어디로 가서 무엇을 언제까지 하면 되는지까지다.
  */
  /*
    K-패스 글. **얼마인가를 말하는 첫 글이다.** 앞의 글들은 "언제 신청하나·
    언제 들어오나"까지였다.

    왜 이걸 골랐나. 수요가 서로 다른 두 곳에서 겹쳐 나왔다 —
    네이버 데이터랩 12개월 월평균으로 「K패스」가 실업급여의 2.4배이고,
    토스 미니앱 인기 100선에도 K패스 환급 계산이 세 개(53·78·100위) 있다.
    그런데 우리 상세 페이지는 "20%~53.3%"라는 범위만 싣고 있었다.

    이 글도 수록 데이터에서 뽑은 숫자가 하나도 없다. 요율과 기준금액은
    전부 한국교통안전공단(운영기관) 표다 — 복지로가 안 싣는 값을 1차
    출처에서 찾아 메우는 자리(lib/payDates.ts와 같다).
  */
  /*
    근로장려금 금액 글. `/guide/tax-credit`(언제 신청하고 언제 들어오나)과
    **짝을 이룬다** — 저쪽은 언제, 이쪽은 얼마다. 사람들이 치는 말도 갈린다.

    토스 미니앱 인기 100선에서 **단일 주제로 가장 많은 것이 근로장려금**이었다
    (10·22·26·30·55·74위 — 여섯 개).

    다른 계산기 글들과 결정적으로 다른 점: K-패스는 공단 표를, 청년미래적금은
    금융위 보도자료를 밖에서 찾아왔지만 **이건 밖에서 가져온 숫자가 하나도
    없다.** 산식이 우리 수록 원문에 통째로 들어 있었다.
  */
  {
    slug: "tax-credit-amount",
    title: "근로장려금, 얼마 받나",
    summary:
      "소득이 오를수록 늘다가 한동안 그대로였다가 다시 줄어 0이 됩니다. 그 꺾이는 지점이 가구 유형마다 다르고, 재산이 1.7억을 넘으면 절반이 됩니다.",
    updated: "2026-09-15",
  },
  /*
    청년미래적금 글. 원문이 "납입액의 **일정비율**"이라고만 적고 비율을 안 준다
    — K-패스가 "20%~53.3%"라고만 했던 것과 같은 구멍이다(토스 82위).

    이 글에는 다른 글에 없는 절이 하나 있다. **두 공식 출처가 어긋나서**
    (일반형 소득기준이 복지로 7,500만 대 금융위 6,000만), 그 사실 자체를
    본문에 적고 그 구간은 계산하지 않는다.
  */
  {
    slug: "youth-savings",
    title: "청년미래적금, 3년 뒤 얼마가 되나",
    summary:
      "매월 낸 돈에 정부가 6% 또는 12%를 얹어 줍니다. 복지로 안내에는 그 비율이 적혀 있지 않아 금융위원회 자료에서 확인해 옮겼습니다.",
    updated: "2026-09-15",
  },
  {
    slug: "k-pass",
  ...
  /*
    사업별 해설 글 셋 (2026-09-13). 애드센스 「가치가 별로 없는 콘텐츠」 거절 뒤,
    조회수 상위인데 상세 페이지에 **우리가 따로 확인한 것이 0**이던 사업에서 골랐다
    — 1위 청년내일저축계좌, 7위 이동통신요금감면, 6·10·18위 보육료·유아학비·양육수당.

    셋 다 복지로 원문이 아니라 **원문에 이름·첨부로만 걸린 1차 출처**를 읽고 썼다
    (보건복지부 자산형성 사업안내 PDF · 과기정통부 고시 제5조 · 교육부 보육사업안내
    PDF). 글마다 쪽수·조항을 달았다. 글을 쓰면 `lib/serviceExtras.ts`에 사업을
    이어 상세의 「복지클릭이 따로 확인한 것」에 뜨게 한다.
  */
  {
  ...
  /* 2026-09-16 — 수록분을 센 값만으로 쓰는 첫 글 둘. 원문을 옮기지 않고
  ...
  },
  {
    ...
    summary:
      "2026년은 1인당 10만원으로 지난해의 두 배가 됐지만, 쓰는 기간이 7월 13일부터 10월 12일 오후 5시까지입니다. 신청 자격의 출생 연도가 복지로 원문과 다른 점까지 재단 모집 공고와 지원센터 누리집으로 확인했습니다.",
    updated: "2026-09-21",
  },
  {
    slug: "hope-savings",
    ...
    summary:
      "매달 10만원을 넣으면 정부가 Ⅰ은 30만원, Ⅱ는 10~30만원을 얹어 주는 3년 통장입니다. 2026년 남은 모집은 Ⅱ 10월 1~26일, Ⅰ 11월 2~16일이고, 3년을 채워도 탈수급·교육·계획서 조건을 못 맞추면 정부지원금은 돌려받지 못합니다. 보건복지부 2026년 사업안내로 확인했습니다.",
    updated: "2026-09-22",
  },
  {
