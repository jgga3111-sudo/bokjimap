...
/* ── 건강보험료 → 소득 역산 ───────────────────────────────────────
   ...
   ── 근거 (2026-09-07 원문 대조) ─────────────────────────────────
   · **요율 7.19%** — 「국민건강보험법 시행령」 제44조제1항.
     조문은 %가 아니라 **"보험료율은 1만분의 719로 한다"**로 적혀 있다
     (같은 법 제73조제1항의 위임). 2025-08-28 제15차 건강보험정책심의위원회가
     7.09%에서 0.1%p 올려 의결했고(보건복지부 보도자료 「2026년 건강보험료율
     7.19%로 결정」), 2026년 1월분 보험료부터 적용된다.
   · **본인부담 절반** — 「국민건강보험법」 제76조제1항.
     근로자든 공무원이든 사립학교 교원이든 **직장가입자 본인 몫은 100분의 50**
     으로 같다(교원은 나머지를 사용자 30·국가 20으로 나눠 낼 뿐이다).
     그래서 ÷0.03595는 직장가입자 전체에 통한다.

   → 본인부담률 3.595%, 보수월액 = 본인부담 보험료 ÷ 0.03595

   ── 지역가입자에게는 왜 안 주는가 ───────────────────────────────
   지역가입자는 [소득 × 7.19%] + [재산보험료부과점수 × 211.5원] 구조라
   (시행령 제44조제1항·제2항) 보험료만으로 소득을 되짚을 수 없다.
   점수당 금액은 블로그마다 208.4원(2025년 값)으로 적힌 것이 섞여 있다.
   **법제처 찾기쉬운 생활법령정보 2026-08-15 기준 211.5원**이 맞다.

   ── 이 역산이 맞지 않는 구간 ────────────────────────────────────
   보험료에는 월별 상한·하한이 있어, 그 바깥에서는 보험료가 소득을 따라
   움직이지 않는다. 하한에 걸린 사람은 실제보다 소득이 높게 나온다.
   다만 그 구간은 어느 기준선에도 한참 못 미치는 소득이라 화면의 판정은
   바뀌지 않는다. 상한 쪽도 마찬가지로 판정이 "해당 없음"에서 안 바뀐다.

   수치를 고칠 때는 **매년 8~9월 건정심 의결과 시행령 개정을 함께** 본다.
   보도자료만 보고 고치면 실제 조문과 어긋날 수 있다.
   ──────────────────────────────────────────────────────────────── */
export const HEALTH_INSURANCE = {
  /** 2026년 건강보험료율. 시행령 제44조제1항 "1만분의 719"(사업주 몫 포함). */
  rate: 0.0719,
  /** 본인부담분 = 전체의 절반. 법 제76조제1항 "100분의 50". */
  employeeRate: 0.0719 / 2,
  /**
   * 장기요양보험료율(건강보험료 대비). **화면에는 아직 안 쓴다.**
   *
   * 2025-11-04 제6차 장기요양위원회 의결 — 소득 대비 0.9448%,
   * 건강보험료 대비 13.14%(보건복지부 보도자료 「2026년도 장기요양보험료율
   * 0.9448%」). 7.19 × 0.1314 = 0.9448이라 두 표기가 맞물린다.
   *
   * 건강보험료율과 **의결 시기가 다르다**(건정심 8월 / 장기요양위 11월).
   * 한쪽만 고치고 넘어가지 않도록 여기 같이 적어 둔다.
   */
