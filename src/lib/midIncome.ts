 ...
 *
 * 7인 이상은 고시가 표로 주지 않고 "1인 증가할 때마다 999,233원씩 증가"로
 * 규정한다(= 6인 8,555,952 − 5인 7,556,719). 그래서 표에 박지 않고 규칙으로
 * 계산한다. 규칙을 코드로 옮기면 8인·9인도 자동으로 맞는다.
 */
const TABLE = [
  0,
  2_564_238, // 1인
...
/** 7인 이상 가구에서 1인 증가할 때마다 더하는 금액(고시 규정). */
const PER_EXTRA_PERSON = TABLE[6] - TABLE[5]; // 999,233

export const MAX_TABLE_SIZE = TABLE.length - 1; // 6

/** 가구원 수 → 기준 중위소득(월, 원). 7인 이상은 고시 규칙으로 외삽한다. */
